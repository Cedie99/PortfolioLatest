        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
          {designProjects.map((project, idx) => (
            <motion.a
              ...
              className={`${project.span} group relative overflow-hidden rounded-[2rem] md:rounded-[2.5rem] bg-zinc-900 border border-white/5 block ${
                project.tall
                  ? "aspect-[4/3] sm:aspect-video md:aspect-auto md:h-[500px]"
                  : "aspect-[4/3] sm:aspect-video md:aspect-auto md:h-[400px]"
              }`}
            >
              {/* Text overlay */}
              ...
              {/* Stronger gradient + frosted bar */}
              <div className="absolute inset-0 z-10 bg-gradient-to-t from-black/90 via-black/40 to-transparent opacity-90 group-hover:opacity-95 transition-opacity duration-500" />
              <div className="absolute bottom-0 left-0 right-0 h-24 z-10 bg-black/30 backdrop-blur-sm" />

              {/* Image with skeleton */}
              <div className="absolute inset-0 bg-zinc-800 group-hover:scale-110 transition-transform duration-1000 ease-out">
                {project.image ? (
                  <ImageWithSkeleton
                    src={project.image}
                    alt={project.title}
